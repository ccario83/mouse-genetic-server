...
'''
# ==== DEBUG ====
connection = psycopg2.connect(host='ec2-50-17-147-129.compute-1.amazonaws.com', user='ror', password='m1ckeym0use', database='mus_musculus_core_67_37', port=5432)
...
for snp in SNPs:
    ...
    cursor.execute('''   
                    SELECT 
                    	...
                    	g.seq_region_start, 
                    	g.seq_region_end, 
                    	LEAST(ABS(g.seq_region_start - %d), ABS(g.seq_region_end - %d)) as distance
                    FROM gene g
                    ...
                    ORDER BY distance
                    LIMIT 1 
                    ''' % (long(snp['Pos']), long(snp['Pos']), chromo) )

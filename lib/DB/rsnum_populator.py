...
def biomart_vep(SNPs):
    SNPs = ",".join([ "%d:%d:%d:1" % (snp['chromosome'], snp['position'], snp['position']) for snp in SNPs ])
...
while True:
    ...
    print ("Querying SNPs %d - %d\r" % (cur_snp_cut+1, cur_snp_cut+20))

    # Get all the SNP ids from the snp position table
    cursor.execute('SELECT id FROM snp_positions WHERE id > %s LIMIT 20'%cur_snp_cut)

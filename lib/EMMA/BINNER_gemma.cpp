...
int main(int argc, char *argv[])
{
	...
	while(!feof(fhIn))
	{
		...
		{
			// Read the line
			sscanf(line_ptr, "%s\t%d\t%lu\t%lf", snp_ID, &chr, &pos, &p_value);
			...
			*/
			if (chr==23)
			{
				chr = 20; // Fix weird thing plink does when calling X chr 23 for the 65M sanger set
			}
			unsigned long bin = find_bin_number(pos, bin_size);
			
			if (p_value < bins[chr-1][bin].p_value)
			{
				//printf("\nBin Number: %lu", bin);
				//printf("\nOLD:\t%s\t%s\t%lu\t%.20lf", bins[chr][bin].snp_ID, bins[chr][bin].chr, bins[chr][bin].pos, bins[chr][bin].p_value);
				//printf("\nNEW:\t%s\t%s\t%lu\t%.20lf\n", snp_ID, read_chr, pos, p_value);

				//strcpy(bins[chr-1][bin].chr, read_chr);
				bins[chr-1][bin].chr = chr;
				bins[chr-1][bin].pos = pos;
				bins[chr-1][bin].p_value = p_value;
				strcpy(bins[chr-1][bin].snp_ID,snp_ID);
			} 

...
int main( int argc, char *argv[])
{
    ...
    if (!just_parse)
    {
	    ...
	    while(!goldilocks)
	    {
	        ...
	        while(!feof(fhIn))
	        {
		        ...
		        {
			        /* Read the line */
			        sscanf(line_ptr, "%s\t%*s\t%lf", snp_ID, &p_value);
    ...
    while(!feof(fhIn))
    {
	    while(fgets(line, 255, fhIn))
	    {
	        ...
	        strcpy(temp, snp_ID); 
			strtok(temp, "/"); 
			chromosome = strtok(NULL, "/");
            if (strcmp(chromosome, temp_chr))
            {
                if (verbose)
                    { printf("\n\tFiltering chromosome %s...", chromosome); }
                strcpy(temp_chr,chromosome);
            }
		    ...
		    if (p_value <= cutoff)
		    {
		        ...
		        char *decoded_snp_ID;
		        strcpy(temp, snp_ID);
		        decoded_snp_ID = strtok(temp, "/"); // Get the first part of the SNP ID as the actual snp ID
				chromosome = strtok(NULL, "/"); // Get the chromosome part encoded in the SNP ID
				position = strtok(NULL, "/");   // Get the position part encoded in the SNP ID
                //printf("%s\t%s\t%s\t%.20f\n", decoded_snp_ID, chromosome, position, p_value);
                fprintf(fhOut,"%s\t%s\t%s\t%.20f\n", decoded_snp_ID, chromosome, position, p_value);
			}
	    }
    }
